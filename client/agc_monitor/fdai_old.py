...
def anglerange(start, stop, resolution=1, include_stop=True, reverse=False):
    angles = []
    if resolution > 0:
        while stop < start:
            stop += 360
...
class BlackHalf(object):
    def __init__(self):
        ...
        self.polygons = []
        # Loop over 12 15-degree segments east-west
        for i in range(12):
            # loop over 10 15-degree segments north-south
            for j in range(10):
                ...
                # South side
                latitude = lat1
                for ii in anglerange(long1, long2, DEGREE_RESOLUTION, True):
                    longitude = ii
                    points[n, :] = to_cartesian(longitude, latitude)
                    n += 1
                ...
                # East side
                longitude = long2
                for ii in anglerange(lat1, lat2, DEGREE_RESOLUTION, True):
                    latitude = ii
                    points[n, :] = to_cartesian(longitude, latitude)
                    n += 1
                ...
                # North side
                latitude = lat2
                for ii in anglerange(long1, long2, DEGREE_RESOLUTION, True, reverse=True):
                    longitude = ii
                    points[n, :] = to_cartesian(longitude, latitude)
                    n += 1
                ...
                # West side
                longitude = long1
                for ii in anglerange(lat1, lat2, DEGREE_RESOLUTION, True, reverse=True):
                    latitude = ii
                    points[n, :] = to_cartesian(longitude, latitude)
                    n += 1
...
class GimbalLock(object):
    def __init__(self):
        ...
        # Loop over 24 15-degree segments east-west
        for i in range(24):
            north_points = np.zeros((12, 3))
            south_points = np.zeros((12, 3))
            ...
            for ii in anglerange(lat1, lat2, DEGREE_RESOLUTION, True, reverse=True):
                latitude = ii
                north_points[n, :] = to_cartesian(longitude, latitude)
                south_points[n, :] = to_cartesian(longitude, -latitude)
                n += 1
            ...
            self.polygons.append(FDAIPolygon(north_points, Qt.red))
            self.polygons.append(FDAIPolygon(south_points, Qt.red))
...
class Meridian(object):
    def __init__(self, longitude):
        ...
        if self.longitude <= 180:
            for ii in anglerange(-85, 85, 5, False):
                points = self.build_points(ii, ii+5)
                self.polygons.append(FDAIPolygon(points, Qt.black))

        else:
            points = self.build_points(-85, -75)
            self.polygons.append(FDAIPolygon(points, Qt.black))
            for ii in anglerange(-75, 75, 15, False):
                points = self.build_points(ii, ii + 15)
...
class Parallel(object):
    def __init__(self, latitude, color=None, width=None):
        ...
        # Black half
        for ii in anglerange(0, 180, 15, False):
            points_north = []
            points_south = []
            for current_longitude in anglerange(ii, ii+15, DEGREE_RESOLUTION):
        ...
        # White half
        for ii in anglerange(180, 360, 15, False):
            points_north = []
            points_south = []
            for current_longitude in anglerange(ii, ii+15, DEGREE_RESOLUTION):
                points_north.append(move_fixed_distance(0.5 * width, current_longitude, self.latitude, "north"))
...
class FDAIData(object):
    ...
    def get_visible_polygons(self, q):
        ...
        for i in range(self.npolygons):
            pts = points[sum(self.polygon_sizes[:i]):sum(self.polygon_sizes[:i+1])]
            if np.max(pts[:, 0]) > 0.5*R:
                polygons.append(FDAIPolygon(pts, self.polygon_colors[i]))
        return polygons
...
class FDAI(QWidget):
    ...
    def setOrientation(self, roll, yaw, pitch):
        ...
        oga = math.radians(roll)
        mga = math.radians(yaw)
        iga = math.radians(pitch)
    ...
    def drawRollScale(self, painter):
        # Front
        painter.setBrush(Qt.black)
        painter.setPen(Qt.black)
        points = np.zeros((367, 3))
        inner_radius = R*math.cos(math.radians(45))
        print(inner_radius)
        print(self.width())
        for i in range(361):
            x = inner_radius * math.sin(i*2*math.pi/360)
            y = inner_radius * math.cos(i*2*math.pi/360)
            points[i, :] = (0, x, y)

        halfwidth = R + 20
        points[361, :] = (0, 0, halfwidth)
        points[362, :] = (0, halfwidth, halfwidth)
        points[363, :] = (0, halfwidth, -halfwidth)
        points[364, :] = (0, -halfwidth, -halfwidth)
        points[365, :] = (0, -halfwidth, halfwidth)
        points[366, :] = (0, 0, halfwidth)

        p = self.build_path(points, BALL_SCALE)
        painter.setBrush(Qt.black)
        painter.drawPath(p)
        p = self.build_path(points[:-6, :], BALL_SCALE)
    ...
    def drawScale(self, painter):
        ...
        painter.drawEllipse(self.center, r - 26, r - 26)
...
"""
Approach:
    Build point list for all polygons on sphere. Most are small, local polygons: tick, numbers. Some are larger:
    meridians and parallels, and colored regions (red gimbal lock areas, black half). These can be split into smaller
    parts, each not larger than say 15x15 degrees. Then no single polygon extends over greater distance than 15 degrees.
    As only the central 90 degrees of the ball is visible, that leaves
    45 degrees on either side. So we can just cull all object with points closer than sin(30 deg) = 0.5 * radius.
